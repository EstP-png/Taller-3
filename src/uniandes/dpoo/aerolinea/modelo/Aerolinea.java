...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        if (getVuelo(codigoRuta,fecha).equals(null)&& !avioncito.equals(null)) {
        	Vuelo vuelo = new Vuelo(ruta,fecha,avioncito);
        	vuelos.add(vuelo);
        }else {
        	System.out.println("El avion no existe o ya se encuentra ocupado");
        }
    	}catch(Exception e){
    		
    		
    		System.out.println("Problena con los datos suministrados");
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if ((mes >= 1 && mes <= 5)|| (mes >= 9 && mes<= 11)) {
        	return vuelo.venderTiquetes​(getCliente(identificadorCliente), new CalculadoraTarifasTemporadaBaja(), cantidad);
        }
        	return vuelo.venderTiquetes​(getCliente(identificadorCliente), new CalculadoraTarifasTemporadaAlta(), cantidad);
    ...
    {
    	Vuelo vuelo = getVuelo(codigoRuta,fecha);
    	for(Tiquete tiquet : vuelo.getTiquetes()) {

    			tiquet.marcarComoUsado();
    		}
    	}
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        return cliente.calcularValorTotalTiquetes().toString();
    }
